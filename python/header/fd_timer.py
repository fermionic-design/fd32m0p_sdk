...
class INPUT_FILTER_CC_2:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+88
		self.filter_en_2 = 0
		self.is_consecutive_prd_2 = 0
		self.filter_period_2 = 0
	def write(self):
		val = 0
		val |= self.filter_en_2 << 0
		val |= self.is_consecutive_prd_2 << 1
		val |= self.filter_period_2 << 2
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.filter_en_2 = (val & 0x01) >> 0
		self.is_consecutive_prd_2 = (val & 0x02) >> 1
		self.filter_period_2 = (val & 0x0C) >> 2
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class INPUT_FILTER_CC_3:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+92
		self.filter_en_3 = 0
		self.is_consecutive_prd_3 = 0
		self.filter_period_3 = 0
	def write(self):
		val = 0
		val |= self.filter_en_3 << 0
		val |= self.is_consecutive_prd_3 << 1
		val |= self.filter_period_3 << 2
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.filter_en_3 = (val & 0x01) >> 0
		self.is_consecutive_prd_3 = (val & 0x02) >> 1
		self.filter_period_3 = (val & 0x0C) >> 2
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class INPUT_CC_2:
	...
	def write(self):
		val = 0
		val |= self.input_sel_2 << 0
		val |= self.input_inv_2 << 4
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.input_sel_2 = (val & 0x0F) >> 0
		self.input_inv_2 = (val & 0x10) >> 4
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class INPUT_CC_3:
	...
	def write(self):
		val = 0
		val |= self.input_sel_3 << 0
		val |= self.input_inv_3 << 4
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.input_sel_3 = (val & 0x0F) >> 0
		self.input_inv_3 = (val & 0x10) >> 4
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC2_CAPTURE_CTRL:
	...
	def write(self):
		val = 0
		val |= self.cap_cond_2 << 0
		val |= self.load_cond_2 << 2
		val |= self.zero_cond_2 << 4
		val |= self.adv_cond_2 << 6
		val |= self.cap_val_2 << 9
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cap_cond_2 = (val & 0x03) >> 0
		self.load_cond_2 = (val & 0x0C) >> 2
		self.zero_cond_2 = (val & 0x30) >> 4
		self.adv_cond_2 = (val & 0x1C0) >> 6
		self.cap_val_2 = (val & 0x1FFFE00) >> 9
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC3_CAPTURE_CTRL:
	...
	def write(self):
		val = 0
		val |= self.cap_cond_3 << 0
		val |= self.load_cond_3 << 2
		val |= self.zero_cond_3 << 4
		val |= self.adv_cond_3 << 6
		val |= self.cap_val_3 << 9
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cap_cond_3 = (val & 0x03) >> 0
		self.load_cond_3 = (val & 0x0C) >> 2
		self.zero_cond_3 = (val & 0x30) >> 4
		self.adv_cond_3 = (val & 0x1C0) >> 6
		self.cap_val_3 = (val & 0x1FFFE00) >> 9
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC4_CMN_CTRL:
	def __init__(self,dev,base_addr):
		...
		self.suppress_coc_event_gen_4 = 0
	def write(self):
		val = 0
		val |= self.suppress_coc_event_gen_4 << 0
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.suppress_coc_event_gen_4 = (val & 0x01) >> 0
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC5_CMN_CTRL:
	def __init__(self,dev,base_addr):
		...
		self.suppress_coc_event_gen_5 = 0
	def write(self):
		val = 0
		val |= self.suppress_coc_event_gen_5 << 0
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.suppress_coc_event_gen_5 = (val & 0x01) >> 0
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC4_COMPARE_CTRL:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+168
		self.comp_val_4 = 0
		self.comp_val_update_method_4 = 0
	def write(self):
		val = 0
		val |= self.comp_val_4 << 0
		val |= self.comp_val_update_method_4 << 16
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.comp_val_4 = (val & 0xFFFF) >> 0
		self.comp_val_update_method_4 = (val & 0x70000) >> 16
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC5_COMPARE_CTRL:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+172
		self.comp_val_5 = 0
		self.comp_val_update_method_5 = 0
	def write(self):
		val = 0
		val |= self.comp_val_5 << 0
		val |= self.comp_val_update_method_5 << 16
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.comp_val_5 = (val & 0xFFFF) >> 0
		self.comp_val_update_method_5 = (val & 0x70000) >> 16
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC2_CC_PWM_CFG:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+184
		self.cc_out_cc2d_2 = 0
		self.cc_out_cc2u_2 = 0
		self.cc_out_ccd_2 = 0
		self.cc_out_ccu_2 = 0
		self.cc_out_load_2 = 0
		self.cc_out_zero_2 = 0
		self.cc_out_fault_entry_2 = 0
		self.cc_out_fault_exit_2 = 0
	def write(self):
		val = 0
		val |= self.cc_out_cc2d_2 << 0
		val |= self.cc_out_cc2u_2 << 2
		val |= self.cc_out_ccd_2 << 4
		val |= self.cc_out_ccu_2 << 6
		val |= self.cc_out_load_2 << 8
		val |= self.cc_out_zero_2 << 10
		val |= self.cc_out_fault_entry_2 << 12
		val |= self.cc_out_fault_exit_2 << 15
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cc_out_cc2d_2 = (val & 0x03) >> 0
		self.cc_out_cc2u_2 = (val & 0x0C) >> 2
		self.cc_out_ccd_2 = (val & 0x30) >> 4
		self.cc_out_ccu_2 = (val & 0xC0) >> 6
		self.cc_out_load_2 = (val & 0x300) >> 8
		self.cc_out_zero_2 = (val & 0xC00) >> 10
		self.cc_out_fault_entry_2 = (val & 0x7000) >> 12
		self.cc_out_fault_exit_2 = (val & 0x38000) >> 15
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC3_CC_PWM_CFG:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+188
		self.cc_out_cc2d_3 = 0
		self.cc_out_cc2u_3 = 0
		self.cc_out_ccd_3 = 0
		self.cc_out_ccu_3 = 0
		self.cc_out_load_3 = 0
		self.cc_out_zero_3 = 0
		self.cc_out_fault_entry_3 = 0
		self.cc_out_fault_exit_3 = 0
	def write(self):
		val = 0
		val |= self.cc_out_cc2d_3 << 0
		val |= self.cc_out_cc2u_3 << 2
		val |= self.cc_out_ccd_3 << 4
		val |= self.cc_out_ccu_3 << 6
		val |= self.cc_out_load_3 << 8
		val |= self.cc_out_zero_3 << 10
		val |= self.cc_out_fault_entry_3 << 12
		val |= self.cc_out_fault_exit_3 << 15
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cc_out_cc2d_3 = (val & 0x03) >> 0
		self.cc_out_cc2u_3 = (val & 0x0C) >> 2
		self.cc_out_ccd_3 = (val & 0x30) >> 4
		self.cc_out_ccu_3 = (val & 0xC0) >> 6
		self.cc_out_load_3 = (val & 0x300) >> 8
		self.cc_out_zero_3 = (val & 0xC00) >> 10
		self.cc_out_fault_entry_3 = (val & 0x7000) >> 12
		self.cc_out_fault_exit_3 = (val & 0x38000) >> 15
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC2_OUTPUT_CTL:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+212
		self.cc_out_sel_2 = 0
		self.cc_out_inv_2 = 0
		self.cc_out_val_init_2 = 0
	def write(self):
		val = 0
		val |= self.cc_out_sel_2 << 0
		val |= self.cc_out_inv_2 << 4
		val |= self.cc_out_val_init_2 << 5
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cc_out_sel_2 = (val & 0x0F) >> 0
		self.cc_out_inv_2 = (val & 0x10) >> 4
		self.cc_out_val_init_2 = (val & 0x20) >> 5
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC3_OUTPUT_CTL:
	def __init__(self,dev,base_addr):
		self.dev = dev
		self._addr = base_addr+216
		self.cc_out_sel_3 = 0
		self.cc_out_inv_3 = 0
		self.cc_out_val_init_3 = 0
	def write(self):
		val = 0
		val |= self.cc_out_sel_3 << 0
		val |= self.cc_out_inv_3 << 4
		val |= self.cc_out_val_init_3 << 5
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cc_out_sel_3 = (val & 0x0F) >> 0
		self.cc_out_inv_3 = (val & 0x10) >> 4
		self.cc_out_val_init_3 = (val & 0x20) >> 5
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC2_SW_FORCE:
	def __init__(self,dev,base_addr):
		...
		self.cc_out_sw_force_2 = 0
		self.cc_cmpl_out_sw_force_2 = 0
	def write(self):
		val = 0
		val |= self.cc_out_sw_force_2 << 0
		val |= self.cc_cmpl_out_sw_force_2 << 2
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cc_out_sw_force_2 = (val & 0x03) >> 0
		self.cc_cmpl_out_sw_force_2 = (val & 0x0C) >> 2
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class CC3_SW_FORCE:
	def __init__(self,dev,base_addr):
		...
		self.cc_out_sw_force_3 = 0
		self.cc_cmpl_out_sw_force_3 = 0
	def write(self):
		val = 0
		val |= self.cc_out_sw_force_3 << 0
		val |= self.cc_cmpl_out_sw_force_3 << 2
		self.dev.write(self._addr,val)
	def read(self):
		val = self.dev.read(self._addr)
		self.cc_out_sw_force_3 = (val & 0x03) >> 0
		self.cc_cmpl_out_sw_force_3 = (val & 0x0C) >> 2
		return val
	def display(self):
		attribute_names = [name for name in dir(self) if not callable(getattr(self, name)) and not name.startswith("__")]
		for name in attribute_names:
...
class fd_timer:
	def __init__(self,dev,base_addr):
		...
		self.INPUT_FILTER_CC_2 = INPUT_FILTER_CC_2(dev,base_addr)
		self.INPUT_FILTER_CC_3 = INPUT_FILTER_CC_3(dev,base_addr)
		self.INPUT_CC_0 = INPUT_CC_0(dev,base_addr)
		self.INPUT_CC_1 = INPUT_CC_1(dev,base_addr)
		self.INPUT_CC_2 = INPUT_CC_2(dev,base_addr)
		self.INPUT_CC_3 = INPUT_CC_3(dev,base_addr)
		...
		self.CC2_CAPTURE_CTRL = CC2_CAPTURE_CTRL(dev,base_addr)
		self.CC3_CAPTURE_CTRL = CC3_CAPTURE_CTRL(dev,base_addr)
		self.CC2_COMPARE_CTRL = CC2_COMPARE_CTRL(dev,base_addr)
		self.CC3_COMPARE_CTRL = CC3_COMPARE_CTRL(dev,base_addr)
		self.CC4_CMN_CTRL = CC4_CMN_CTRL(dev,base_addr)
		self.CC5_CMN_CTRL = CC5_CMN_CTRL(dev,base_addr)
		self.CC4_COMPARE_CTRL = CC4_COMPARE_CTRL(dev,base_addr)
		self.CC5_COMPARE_CTRL = CC5_COMPARE_CTRL(dev,base_addr)
		self.CC0_CC_PWM_CFG = CC0_CC_PWM_CFG(dev,base_addr)
		self.CC1_CC_PWM_CFG = CC1_CC_PWM_CFG(dev,base_addr)
		self.CC2_CC_PWM_CFG = CC2_CC_PWM_CFG(dev,base_addr)
		self.CC3_CC_PWM_CFG = CC3_CC_PWM_CFG(dev,base_addr)
		...
		self.CC2_OUTPUT_CTL = CC2_OUTPUT_CTL(dev,base_addr)
		self.CC3_OUTPUT_CTL = CC3_OUTPUT_CTL(dev,base_addr)
		self.CC2_SW_FORCE = CC2_SW_FORCE(dev,base_addr)
		self.CC3_SW_FORCE = CC3_SW_FORCE(dev,base_addr)
