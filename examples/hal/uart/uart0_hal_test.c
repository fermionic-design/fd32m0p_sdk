...
//******************************UART1******************************************
   ...
   //configuring UART1 for printing, 1.5 MHz baud rate, 32 MHz clk freq
    ...
    uart_puts(UART1_REGS, "UART_HAL_TEST, configured UART 1 for printing\n");
...
//********************************UART0*****************************************
    ...
    uart0_cfg_struct.tx_en = UART_CTRL_TX_EN_HW;
        ...
        uart0_cfg_struct.loopback_en = 1;
    #endif
    ...
    iomux_cfg_struct_tx.output_en        = 1;
    iomux_cfg_struct_tx.input_en         = 0;
    iomux_cfg_struct_tx.sel              = IOMUX_PIN_SEL_PA17_UART0_TX;

    iomux_cfg(IOMUX_REGS, iomux_cfg_struct_tx, 17);

    /* uncomment and program proper iomux pin cfg
     //rx port
     iomux_cfg_struct_rx.output_en        = 0;
     iomux_cfg_struct_rx.input_en         = 1;
     iomux_cfg_struct_rx.sel              = IOMUX_PIN_SEL_PA18_UART0_RX;

     iomux_cfg(IOMUX_REGS, iomux_cfg_struct_rx, 18;
    
     //cts port
     iomux_cfg_struct_cts.output_en        = 0;
     iomux_cfg_struct_cts.input_en         = 1;
     iomux_cfg_struct_cts.sel              = IOMUX_PIN_SEL_PA14_UART0_CTS;
     ...
     iomux_cfg_struct_rts.sel              = IOMUX_PIN_SEL_PA15_UART0_RTS;
    ...
    //configure UART0 fifo
    uart0_fifo_cfg_struct.fifo_en = 1;

    uart_fifo_cfg(UART0_REGS, &uart0_fifo_cfg_struct);


//************************data transmit*******************************************
    uart_puts(UART1_REGS, "UART_HAL_TEST writing data\n");
    ...
    UART_INTR_EVENT_EN(UART0_REGS, UART_INTR_EVENT_TX_INT_IDX);   

    //checker
    #ifdef LPBK
        #ifdef uart_rxdrain_blocking
            uart_rxfifo_drain_blocking(UART0_REGS, data_rx_arr, DATA_LEN);
        #else
            for(int j=0; j<DATA_LEN; j++)
            {
                while((UART0_REGS->FIFOSTS.rx_fifo_empty_sts) == 1);
                data_rx_arr[j] = uart_getc(UART0_REGS);   
            }
        #endif
...
{
    int intr_sts;
    intr_sts = UART0_REGS->INTR_STS.packed_w-1;
    
    //tx and rx static
    #ifdef UART_HAL_TX_STATIC
        if(intr_sts == UART_INTR_EVENT_TX_INT_IDX)
        {
            txn_done = uart_txfifo_fill_static_nonblocking(UART0_REGS, mem_8, DATA_LEN, rst_int_ctr);
        }

        UART_INTR_EVENT_CLEAR(UART0_REGS, UART_INTR_EVENT_TX_INT_IDX);
        if(txn_done == 1)
            rst_int_ctr = 1;
        else
            rst_int_ctr = 0;
        ...
        if(intr_sts == UART_INTR_EVENT_TX_INT_IDX) 
        {
            num_bytes_written = uart_txfifo_fill_nonblocking(UART0_REGS, &mem_8[0 + num_bytes_wr_int], (DATA_LEN-num_bytes_wr_int));
        }
